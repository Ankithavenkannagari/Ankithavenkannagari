# 👋 Hello, I am Ankitha Venkannagari
**Data Analyst | Power BI Enthusiast | Cloud & Big Data Learner**

Thanks for stopping by! Here's a little bit about me:

🌱 I’m currently learning **Advanced Data Analytics**, **Azure Data Services**, and **Cloud Automation**  
💬 Ask me about **Power BI**, **SQL**, **Python**, **ETL Pipelines**, **Cloud Analytics**  
👯 I’m looking to collaborate on **Data Analytics & BI Projects**  
💻 All of my projects are available at [Portfolio](https://ankithavenkannagari.github.io/)  
📫 Reach me at: **ankithareddy1331@gmail.com** | [LinkedIn](https://www.linkedin.com/in/reddy-ankitha/)  

---

### 🛠️ Languages and Tools
`SQL` `Python` `Power BI` `Tableau` `Excel` `DAX` `Power Query` `Azure` `AWS` `Git` `Linux` `MySQL` `Snowflake`  

---

### 💻 Currently Learning
- Cloud-based ETL Automation  
- Advanced Power BI & Dashboarding Techniques  
- Big Data Analytics & Azure Synapse  
- Data Warehousing in Snowflake  

---

### 🤘 Recent Projects
- **Sales Dashboard & KPI Analytics** – Built interactive dashboards in Power BI for retail sales analysis  
- **Customer Segmentation** – SQL & Python project for customer behavior analysis  
- **ETL Pipeline for Retail Data** – Automated data ingestion & transformation in Azure  
- **Supply Chain Analytics Automation** – Power BI dashboards integrated with automated ETL pipelines  

---

### 🏆 Certifications & Courses
- Microsoft Certified: **Azure Fundamentals**  
- Microsoft Certified: **Data Analyst Associate**  
- AWS Certified: **Cloud Practitioner**   

---

### 📈 GitHub Stats
![Ankitha's GitHub stats](https://github-readme-stats.vercel.app/api?username=ankithareddy&show_icons=true&theme=radical)  

---

### 🌟 Fun Facts
- ☕ Coffee-fueled data explorer  
- 🎯 Always optimizing dashboards for insights  
- 📊 Love turning raw data into meaningful stories  

---

### 📫 Connect with me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?logo=linkedin&style=flat-square)](https://www.linkedin.com/in/reddy-ankitha/)  
[![Email](https://img.shields.io/badge/Email-Write%20Me-red?style=flat-square)](mailto:ankithareddy1331@gmail.com)
